...
class Controller:
    ...
    def handleCompConnessa(self,e):
        ...
        if not self._model.hasNode(idOggetto):
            ...
            self._view.txt_result.controls.append(
                ft.Text("Attenzione l'id inseirto non è presente nel grafo", color="red"))
            self._view.update_page()
            return
        sizeCompConn = self._model.getInfoCompConnessa(idOggetto)
        self._view.txt_result.controls.clear()
        self._view.txt_result.controls.append(
            ft.Text(f"La componente connessa contendente l'oggetto con id {idOggetto} è composta di {sizeCompConn} nodi", color="green"))
        ...
        lunValuesDD = list(map(lambda x: ft.dropdown.Option(x), lunValues)) # questo sostituisce il ciclo sopra
    ...
    def handleCerca(self, e):
        ...
        if lun is None:
            self._view.txt_result.controls.clear()
            self._view.txt_result.controls.append(ft.Text("Attenzione selezionare un valore nel campo di lunghezza tra "
                                                          "le scelte proposte", color="red"))
            self._view.update_page()
            return
        ...
        self._view.txt_result.controls.clear()
        self._view.txt_result.controls.append(ft.Text(f"Ho trovato un cammino che parte da {source} e ha un peso pari a {cost}", color="green"))
        self._view.txt_result.controls.append(
