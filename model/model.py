...
class Model:
    ...
    def _ricorsione(self, parziale, lun):
        if len(parziale)== lun:
            # condizione di terminazione, allora parziale è lungo esattamente lun
    ...
    def getInfoCompConnessa(self, id_oggetto):
        ...
        print(f"size connessa con dfs_tree", len(dfsTree.nodes()))
        # strategia 2
        dfsPred = nx.dfs_predecessors(self._graph, source)
        print(f"size connessa con dfs_predecessors ", len(dfsPred.values()))
        # strategia 3 quella piu usata
    ...
    # costruire un grafico quindi metto prima nel inti e poi faccio il def
    def buildGraph(self):
        # aggiungere i nodi, per inserire i nodi devo fare una query
